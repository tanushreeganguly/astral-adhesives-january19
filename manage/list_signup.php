...
if($title){
	$select     = $objTypes->select("tbl_signup", "*", "status = :status AND signup_email LIKE :signup_email", $where, "id DESC");
}
else{
	$select     = $objTypes->select("tbl_signup", "*", "", "", "id DESC");
}
...
if($title){
    $TypeArray = $objTypes->select("tbl_signup", "*", "status = :status AND signup_email LIKE :signup_email", $where, "id DESC", $limit);
}
else{
    $TypeArray = $objTypes->select("tbl_signup", "*", "", "", "id DESC", $limit);
}
?>
            ...
            <table id="example2" class="table table-bordered table-hover" >
                 ...
                 <div class="input-group">
                  <input type="text" name="title" placeholder="Email..." class="form-control" value="<?=$title?>">
                      <span class="input-group-btn">
					   ...
					   &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;
					   <a href="<?=base_url?>export_newsletter.php?search=<?=$title?>" style="font-size: large;"  >Export</a>
                </div>
                </th></tr>
                <tr>
			  ...
			  ?>
                <tr>
                  ...
                  <td><a href="act_signup.php?id=<?=$val['id']?>&status=<?=$val['status']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['status'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                 
                  <a href="act_signup.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_signup.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_signup.php';
			document.frmListing.submit();
		}
	}
	else
