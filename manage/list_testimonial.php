...
if($title){
	$select     = $objTypes->select("tbl_testimonial", "*", "is_delete = :is_delete AND title LIKE :title", $where, "title ASC");
}
else{
	$select     = $objTypes->select("tbl_testimonial", "*", "is_delete = :is_delete", $where, "title ASC");
}
...
if($title){
    $TypeArray = $objTypes->select("tbl_testimonial", "*", "is_delete = :is_delete AND title LIKE :title", $where, "title ASC", $limit);
}
else{
    $TypeArray = $objTypes->select("tbl_testimonial", "*", "is_delete = :is_delete", $where, "title ASC", $limit);
}
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Testimonial <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_testimonial.php"><i class="fa fa-plus-square"></i> Add Testimonial</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
              ...
              <tr>
                ...
                <tr>
                  ...
                  <th width="4%">#</th>
                  <th width="35%">Testimonial Title</th>
                  <th width="25%">Testimonial Image</th>
				  <th width="25%">Youtube Link</th>
			  ...
			  ?>
                <tr>
                  ...
                  if($val['image'] != ""){
                      ?>
                      <img src="../uploads/testimonial_images/<?php echo $val['image']; ?>" width="75px" height="75px" />
                      <?php
                  }
                  ?></td><td>
				  <?php
                  if($val['youtube'] != ""){?>
                      <?=stripslashes($val['youtube'])?>
                  <?php }?></td>
                  <td><a href="act_testimonial.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_testimonial.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_testimonial.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_testimonial.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_testimonial.php';
			document.frmListing.submit();
		}
	}
	else
