...
if($title){
	$select     = $objTypes->select("tbl_faq", "*", "is_delete = :is_delete AND title LIKE :title", $where, "title ASC");
}
else{
	$select     = $objTypes->select("tbl_faq", "*", "is_delete = :is_delete", $where, "title ASC");
}
...
if($title){
    $TypeArray = $objTypes->select("tbl_faq", "*", "is_delete = :is_delete AND title LIKE :title", $where, "title ASC", $limit);
}
else{
    $TypeArray = $objTypes->select("tbl_faq", "*", "is_delete = :is_delete", $where, "title ASC", $limit);
}
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Faq<small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_faq.php"><i class="fa fa-plus-square"></i> Add Faq</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
			  ...
			  ?>
                <tr>
                  ...
                  <td><a href="act_faq.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_faq.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_faq.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_faq.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_faq.php';
			document.frmListing.submit();
		}
	}
	else
