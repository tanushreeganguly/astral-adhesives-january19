...
if(($POST['SAVE']=="SAVE"))
{
	...
	if($insert_id > 0){
		$counter = 0;
				if(count($_FILES['mul_image']['name']) > 0) 
				{
					...
					foreach($_FILES["mul_image"]["tmp_name"] as $key=>$tmp_name){
						echo $temp = $_FILES["mul_image"]["tmp_name"][$key];
		...
		/*if(isset($_FILES['image']['name']) && $_FILES['image']['name'] != ""){
			...
			if($_FILES['image']['size'] > 3097152){
				header("location:add_homeadhesive.php?sysmsg=16&id=".$insert_id);
                exit();
            }

            if(in_array($ext, $validatefiles) == false){
                header("location:add_homeadhesive.php?sysmsg=11&id=".$insert_id);
                exit();
            }
		...
		if(isset($_FILES['image1']['name']) && $_FILES['image1']['name'] != ""){
			...
			if(move_uploaded_file($_FILES['image1']['tmp_name'], "../uploads/astral_adhesive_image/large/".$filename1))
			{
				...
				$main_image1 = "../uploads/astral_adhesive_image/large/main_".$filename1;
				$main_width	= "355";
				$main_height= "200";
			...
			header("location:list_homeadhesive.php?sysmsg=1000&id=".$insert_id);
			exit();
		}
		if($POST['store_image1']=="" || $POST['store_image']==""){
				
			header("location:list_homeadhesive.php?sysmsg=3&pgNo=".$pgNo);
			
			exit();
		}else{
			if($filename1!="" ||$filename!=""){
				header("location:list_homeadhesive.php?sysmsg=1000&pgNo=".$pgNo);
			}else{
				header("location:list_homeadhesive.php?sysmsg=1001&pgNo=".$pgNo);
				exit();
			}
		}
	}	
...
if(($_REQUEST['param']<>"") && ($_REQUEST['id'] <> "")){
    ...
    if($delete){
        header("location:list_homeadhesive.php?sysmsg=1018&pgNo=".$pgNo);
		exit();
    }
}
...
if(($_REQUEST['status']<>"") && ($_REQUEST['id'] <> "")){
	...
	if($update){
		...
		header("location:list_homeadhesive.php?sysmsg=4&pgNo=".$pgNo);
		exit();
	}
}
...
if(($_REQUEST['action']=="delete") && ($_REQUEST['id'] <> ""))
{
	...
	if($delete){
		header("location:list_homeadhesive.php?sysmsg=1002&pgNo=$pgNo");
		exit();
	}else{
		header("location:list_homeadhesive.php?sysmsg=4&pgNo=$pgNo");
		exit();
...
if(($POST['action']=="activeall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_homeadhesive.php?sysmsg=8&pgNo=$pgNo");
		exit();
	}
	...
	if($update > 0){
		header("location:list_homeadhesive.php?sysmsg=1012&pgNo=$pgNo");
		exit();
	}
    else{
		header("location:list_homeadhesive.php?sysmsg=13&pgNo=$pgNo");
		exit();
	}
}
...
if(($POST['action']=="deactiveall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_homeadhesive.php?sysmsg=9&pgNo=$pgNo");
		exit();
	}
    $Delete = implode(',',$DelCheckBox);
	...
	if($update > 0){
		header("location:list_homeadhesive.php?sysmsg=1013&pgNo=$pgNo");
		exit();
	}
    else{
		header("location:list_homeadhesive.php?sysmsg=14&pgNo=$pgNo");
		exit();
	}
}
...
if(($POST['action']=="deleteall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_homeadhesive.php?sysmsg=10");
		exit();
	}
	$Delete = implode(',',$DelCheckBox);
	...
	if($update > 0){
		header("location:list_homeadhesive.php?sysmsg=1014");
		exit();
	}
    else{
		header("location:list_homeadhesive.php?sysmsg=4");
		exit();
	}
}
