...
$where      = array(":is_delete" => '1', ":type" =>'image');
...
if($title){
	$select     = $objTypes->select("tbl_adhesive", "*", "is_delete = :is_delete AND type=:type AND title1 LIKE :title1 OR title2 LIKE :title2", $where, "id DESC");
}
else{
	$select     = $objTypes->select("tbl_adhesive", "*", "is_delete = :is_delete AND type=:type", $where, "id DESC");
}
...
if($title){
    $TypeArray = $objTypes->select("tbl_adhesive", "*", "is_delete = :is_delete AND type=:type AND title1 LIKE :title1 OR title2 LIKE :title2", $where, "id DESC", $limit);
}
else{
    $TypeArray = $objTypes->select("tbl_adhesive", "*", "is_delete = :is_delete AND type=:type", $where, "id DESC", $limit);
}
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Astral Adhesive <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_image.php"><i class="fa fa-plus-square"></i> Add Astral Adhesive</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
              ...
              <tr>
                ...
                <tr>
                  ...
                  <th width="4%">#</th>
                  <th width="35%">Title1</th>
				  <th width="35%">Title2</th>
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=$sino;?></td>
				  <td><?=stripslashes($val['title1'])?></td>
				  <td><?=stripslashes($val['title2'])?></td>
                  
                  <!--<td><?php
                  if($val['image'] != ""){
                      ?>
                      <img src="../uploads/astral_adhesive_image/small/<?php echo $val['image']; ?>" width="100px"  />
                  ...
                  if($val['image1'] != ""){
                      ?>
                      <img src="../uploads/astral_adhesive_image/large/<?php echo $val['image1']; ?>" width="100px"  />
					 
                      <?php
                  }
                  ?></td>
                  <td><a href="act_image.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_image.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_image.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_image.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_image.php';
			document.frmListing.submit();
		}
	}
	else
