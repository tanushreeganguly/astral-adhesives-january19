...
if($title){
	$select     = $objTypes->select("tbl_event", "*", "is_delete = :is_delete AND title LIKE :title", $where, "id DESC");
}else{
	$select     = $objTypes->select("tbl_event", "*", "is_delete = :is_delete", $where, "id DESC");
}
...
if($title){
    $TypeArray = $objTypes->select("tbl_event", "*", "is_delete = :is_delete AND title LIKE :title", $where, "id DESC", $limit);
}else{  
	$TypeArray = $objTypes->select("tbl_event", "*", "is_delete = :is_delete", $where, "id DESC", $limit);
}
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Event<small></small> </h1>
    <ol class="breadcrumb">
     <li><a href="add_event.php"><i class="fa fa-plus-square"></i> Add Event</a></li>
    </ol>
  </section>
  <!-- Main content -->
