...
$mode 		= ($id<>'0') ? 'Edit' : 'Add';
$params     = array(":id" => $id);
$TypeArray	= $objTypes->fetchRow("SELECT id,title,envelope,type_download FROM tbl_newsletter WHERE id = :id", $params);
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> <?=$mode?> Download <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="list_newsletter.php"><i class="fa  fa-navicon"></i>Add Download</a></li>
    </ol>
  </section>
  <!-- Main content -->
           ...
           <input type="hidden" name="pgNo" id="pgNo" value="<?=$_REQUEST['pgNo']?>"  />
            <div class="box-body">
				 ...
				 <div class="form-group">
                  ...
                  <select class="form-control" name="type_download" id="type_download" style="width: 40%">
                      <option value="">Select Type</option>
                       <option value="Brochures"<?php if($TypeArray['type_download']=="Brochures") echo "selected";?>>Brochures</option>
                       <option value="Newsletter"<?php if($TypeArray['type_download']=="Newsletter") echo "selected";?>>Newsletter</option>
                  </select>
              </div>
				  <div class="form-group">
...
function validateForm(){
	...
	if($("#type_download").val()==""){
    $(".errorDiv").show().fadeOut(4000);
    $('#errormessage').text("Please enter download type");
    $("#type_download").focus();
    return false;
  }
