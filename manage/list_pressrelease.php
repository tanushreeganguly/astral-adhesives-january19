...
if($title){
	$select     = $objTypes->select("tbl_pressrelease", "*", "is_delete = :is_delete AND title LIKE :title", $where, "id DESC");
}else{
	$select     = $objTypes->select("tbl_pressrelease", "*", "is_delete = :is_delete", $where, "id DESC");
}
...
if($title){
    $TypeArray = $objTypes->select("tbl_pressrelease", "*", "is_delete = :is_delete AND title LIKE :title", $where, "id DESC", $limit);
}else{  
	$TypeArray = $objTypes->select("tbl_pressrelease", "*", "is_delete = :is_delete", $where, "id DESC", $limit);
}
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Press release<small></small> </h1>
    <ol class="breadcrumb">
     <li><a href="add_pressrelease.php"><i class="fa fa-plus-square"></i> Add Press release</a></li>
    </ol>
  </section>
  <!-- Main content -->
			...
			<tr>
			  ...
			  ?>
                <tr>
                  ...
                  <td>
                  <a href="add_pressrelease.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
