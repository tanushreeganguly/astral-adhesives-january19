...
if($title){
	$select     = $objTypes->select("tbl_brands", "*", "is_delete = :is_delete AND title LIKE :title1", $where, "id DESC");
}
else{
	$select     = $objTypes->select("tbl_brands", "*", "is_delete = :is_delete", $where, "id DESC");
}
...
if($POST['setpriority'] == 'Sort Order'){  
	...
	header("location:list_brand.php?sysmsg=1001&category=".$POST['category']);
	exit();
}
if($title){
    $TypeArray = $objTypes->select("tbl_brands", "*", "is_delete = :is_delete AND title LIKE :title1", $where, "id DESC", $limit);
}
else{
    $TypeArray = $objTypes->select("tbl_brands", "*", "is_delete = :is_delete", $where, "id DESC", $limit);
}
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Brands <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_brand.php"><i class="fa fa-plus-square"></i> Add Brand</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
			  ...
			  ?>
                <tr>
                  ...
                  if($val['image'] != ""){
                      ?>
                      <img src="../uploads/brand_images/<?php echo $val['image']; ?>" height="55px"  />					 
                      <?php
                  }
                  ?></td>
                  ...
                  if($val['image1'] != ""){
                      ?>
                      <img src="../uploads/brand_images/<?php echo $val['image1']; ?>" height="55px"  />					 
                      <?php
                  }
                  ?></td>
				  <td align="center">                  
                  ...
                  <td><a href="act_brand.php?id=<?=$val['id']?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> </td>
                  <td>
                  <a href="add_brand.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                 <!-- <a href="act_brand.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>-->
                  </td>
                </tr>
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_brand.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_brand.php';
			document.frmListing.submit();
		}
	}
	else
