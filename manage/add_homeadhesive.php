...
$mode 		= ($id<>'0') ? 'Edit' : 'Add';
$params     = array(":id" => $id);
$TypeArray	= $objTypes->fetchRow("SELECT id, title1, title2, image, thumbnail, image1,location,youtube, thumbnail1, added_by, updated_by, updated_date FROM tbl_adhesive WHERE id = :id", $params);
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      <li><a href="list_homebanner.php"><i class="fa  fa-navicon"></i> Astral Adhesive List</a></li>
    </ol>
  </section>
  <!-- Main content -->
    ...
    <div class="row">
      <div class="col-md-12">
        ...
        <div class="box box-primary" >
          ...
          <form role="form" method="post" action="act_homeadhesive.php" onsubmit="return validateForm();" enctype="multipart/form-data">
           ...
           <input type="hidden" name="pgNo" id="pgNo" value="<?=$_REQUEST['pgNo']?>"  />
            <div class="box-body">
				...
				<div class="form-group">
				  ...
				  <div class="form-group">
						<label for="exampleInputEmail1">Image1<?=MANDATORY?></label>
						...
						<a href="#" id='existing_image1'><img src="../uploads/astral_adhesive_image/large/<?=stripslashes($TypeArray['image1'])?>"  onerror="this.style.display='none'" width="100" onclick='window.open("../uploads/astral_adhesive_image/<?=stripslashes($TypeArray['image1'])?>","","width=600,height=600,scrollbars=Yes,resizable=yes")' /></a>
						<?php } ?>
				  ...
				  <div class="form-group">
						...
						<input type="file" class="form-control " id="mul_image" name="mul_image[]" value="" placeholder="Multiple Image" style="width:40%;" multiple="multiple">
						<div class="alert alert-danger alert-dismissible" style="width:40%;margin-top:10px;">[Note:- File Size : 280 x 250  , MAX File Upload Size : 3MB]</div>
						<?php      
					$params_img   = array(":event_id" => $TypeArray['id']);
					$TypeArray_images	= $objTypes->fetchAll("SELECT * FROM tbl_gallery WHERE event_id = :event_id", $params_img);
					foreach($TypeArray_images as $mu_image){
                      ?>
                      <div class="img-wrap2">
                        <span class="close2">&times;</span>
                        <img src="../uploads/astral_adhesive_image/small/<?=stripslashes($mu_image['image'])?>" data-id="<?php echo $mu_image['id'] ?>"  onerror="this.style.display='none'"  width="100" onclick='window.open("../uploads/astral_adhesive_image/small/<?=stripslashes($mu_image['image'])?>","","width=600,height=600,scrollbars=Yes,resizable=yes")' />
					  </div>
                      <?php
                  }
                  ?>
            ...
            <div class="box-footer">
              ...
              <a href="list_homeadhesive.php" class="btn btn-danger" >Back</a>
            </div>
          </form>
        </div>
        <!-- /.box -->
...
jQuery(function(){
    jQuery('.img-wrap2 .close2').click(function() {
        ...
        if(confirm('Are you sure you want to delete selected images?')) {
           	window.location.href = '<?=base_url?>act_homeadhesive.php?id='+id+'&param=rimg&prodid=<?php echo $id ?>&pgNo=<?=$_REQUEST['pgNo']?>';
