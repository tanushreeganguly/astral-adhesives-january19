...
$mode 		= ($id<>'0') ? 'Edit' : 'Add';
$params     = array(":id" => $id);
$TypeArray	= $objTypes->fetchRow("SELECT id, title1, title2, description, image, image1, image2, thumbnail, added_by, updated_by, updated_date FROM tbl_homebanner WHERE id = :id", $params);
           ...
           <input type="hidden" name="pgNo" id="pgNo" value="<?=$_REQUEST['pgNo']?>"  />
            <div class="box-body">
				  ...
				  <div class="form-group">
						...
						<textarea class="form-control" placeholder="Short Description..." name="description" id="editor1" rows="3" style="width:40%;"><?=stripslashes($TypeArray['description'])?></textarea><p id="cnt"></p>
				  </div>
				  <div class="form-group">
					<label for="exampleInputEmail1">External Url</label>
					<input type="url" class="form-control " id="externalurl" name="externalurl" value="<?=stripslashes($TypeArray['externalurl'])?>" placeholder="External Url" style="width:40%;">
				  </div>
				  <div class="form-group">
						...
						<a href="#" id='existing_image'><img src="../uploads/homebanner_images/large/<?=stripslashes($TypeArray['image'])?>"  onerror="this.style.display='none'" width="100" onclick='window.open("../uploads/homebanner_images/large/<?=stripslashes($TypeArray['image'])?>","","width=600,height=600,scrollbars=Yes,resizable=yes")' /></a>
						<?php } ?>
				  </div>
				  <div class="form-group">
						...
						<a href="#" id='existing_image1'><img src="../uploads/homebanner_images/medium/<?=stripslashes($TypeArray['image1'])?>"  onerror="this.style.display='none'" width="100" onclick='window.open("../uploads/homebanner_images/medium/<?=stripslashes($TypeArray['image1'])?>","","width=600,height=600,scrollbars=Yes,resizable=yes")' /></a>
						<?php } ?>
				  </div>
				   <div class="form-group">
						...
						<?php if($TypeArray['image2']){ ?>
						<a href="#" id='existing_image2'><img src="../uploads/homebanner_images/small/<?=stripslashes($TypeArray['image2'])?>"  onerror="this.style.display='none'" width="100" onclick='window.open("../uploads/homebanner_images/small/<?=stripslashes($TypeArray['image2'])?>","","width=600,height=600,scrollbars=Yes,resizable=yes")' /></a>
						<?php } ?>
				  </div>
				   ...
				   <input type="hidden" name="store_image2" value="<?php echo $TypeArray['image2']; ?>">
            </div>
            <!-- /.box-body -->
...
function validateForm(){
	...
	*/
	...
	var image_name2 = '<?php echo  $TypeArray['image2'] ?>';
	if(image_name2 !='')
	{
