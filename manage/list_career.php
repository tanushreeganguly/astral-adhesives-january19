...
$EventArr	= $objTypes->fetchAll("SELECT id FROM tbl_careers WHERE ".$condition." ORDER BY id DESC");
...
$EventArr	= $objTypes->fetchAll("SELECT * FROM tbl_careers WHERE ".$condition." ORDER BY id DESC LIMIT ". $limit);
?>
...
<div class="content-wrapper">
  ...
  <section class="content-header">
    <h1> List Career <small></small> </h1>
    <ol class="breadcrumb">
      <li><a href="add_career.php"><i class="fa fa-plus-square"></i> Add Career</a></li>
    </ol>
  </section>
  <!-- Main content -->
            ...
            <table id="example2" class="table table-bordered table-hover" >
              ...
              <tr>
                ...
                <tr>
                  ...
                  <th width="15%">Job Title</th>
                    <th width="15%">Function/Department</th>
                  <th width="15%">Eduction</th>
			  ...
			  ?>
                <tr>
                  ...
                  <td><?=stripslashes($val['title'])?> <br /> <?=stripslashes($val['location'])?> </td>
                  <td><?=stripslashes($val['department'])?></td>
                  <td><?=stripslashes($val['education'])?></td>
                  <td><?=stripslashes($val['from_experience']).'-'.stripslashes($val['to_experience'])?></td>
                   <td><?=stripslashes($val['from_date'])?> <br />To  <br /> <?=stripslashes($val['to_date'])?> </td>
                   
                  
                  
                  <td><a href="act_career.php?id=<?=$val['id']?>&cat_type=<?=$cat_type?>&title=<?=$title?>&status=<?=$val['is_active']?>&pgNo=<?=base64_encode($pgNo)?>">
                    <?=($val['is_active'] == "1") ? "<span class='label label-success'>Active</span>":"<span class='label label-danger'>Inactive</span>" ?>
                    </a> 
                  </td>
                  <td>
                  <a href="add_career.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>"><i class="fa  fa-edit"></i></a>&nbsp;&nbsp;&nbsp;
                  <a href="act_career.php?id=<?=$val['id']?>&pgNo=<?=base64_encode($pgNo)?>&cat_type=<?=$cat_type?>&title=<?=$title?>&action=delete" onclick="return window.confirm('Do you want to delete this record?')"><i class="fa  fa-trash"></i></a>
                  </td>
                </tr>
                <?php } }  else { print "<tr><td colspan='10'> NO Records Found !!!</td></tr>";} ?>
              </tbody>
              <tr><td colspan="4">
...
function Delete_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Delete Selected Record ?."))
		{
			...
			document.frmListing.action='act_career.php?action=deleteall';
			document.frmListing.submit();
		}
	}
	else
...
function Active_CheckBox()
{
	...
	if(numberOfChecked>=1)
	{
		if(confirm("Are you sure  want to Activate Selected Record ?."))
		{
			...
			document.frmListing.action='act_career.php';
			document.frmListing.submit();
		}
	}
	else
