...
if(($_REQUEST['status']<>"") && ($_REQUEST['id'] <> "")){
	...
	$update     = $objTypes->update("tbl_pressrelease", $params, "id = :id", $where);
	if($update){
		header("location:list_pressrelease.php?sysmsg=1001&pgNo=$pgNo&title=$title");
		exit();
	}
    else{
		header("location:list_pressrelease.php?sysmsg=4&pgNo=$pgNo&title=$title");
		exit();
	}
}
...
if(($_REQUEST['action']=="delete") && ($_REQUEST['id'] <> ""))
{
	...
	if($delete){
		header("location:list_pressrelease.php?sysmsg=1002&pgNo=$pgNo&title=$title");
		exit();
	}else{
		header("location:list_pressrelease.php?sysmsg=4&pgNo=$pgNo&title=$title");
		exit();
...
if(($POST['action']=="activeall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_pressrelease.php?sysmsg=8&pgNo=$pgNo&title=$title");
		exit();
	}
	...
	if($update > 0){
		header("location:list_pressrelease.php?sysmsg=1012&pgNo=$pgNo&title=$title");
		exit();
	}
    else{
		header("location:list_pressrelease.php?sysmsg=13&pgNo=$pgNo&title=$title");
		exit();
	}
}
...
if(($POST['action']=="deactiveall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($Result == "0"){
		header("location:list_pressrelease.php?sysmsg=9&pgNo=$pgNo&title=$title");
		exit();
	}
    $Delete = implode(',',$DelCheckBox);
	...
	if($update > 0){
		header("location:list_pressrelease.php?sysmsg=1013&pgNo=$pgNo&title=$title");
		exit();
	}
    else{
		header("location:list_pressrelease.php?sysmsg=14&pgNo=$pgNo&title=$title");
		exit();
	}
}
...
if(($POST['action']=="deleteall") && ($_POST['DelCheckBox'] <> "")){
	...
	if($update > 0){
		header("location:list_pressrelease.php?sysmsg=1014&title=$title");
		exit();
	}
    else{
		header("location:list_pressrelease.php?sysmsg=4&title=$title");
		exit();
	}
}
